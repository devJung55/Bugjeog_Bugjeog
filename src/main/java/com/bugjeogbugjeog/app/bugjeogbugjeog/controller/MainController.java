...
public class MainController {
    ...
    public String mainPage(HttpSession httpSession){
        if(httpSession.getAttribute("memberId") != null) {  // 자영업자 로그인 시 분기처리
            log.info(httpSession.getAttribute("memberId").toString());

        } else if(httpSession.getAttribute("businessId") != null) { // 비지니스 로그인 시 분기처리
            log.info(httpSession.getAttribute("businessId").toString());
            ...
            httpSession.setAttribute("memberId", memberService.loginNaverOauth(memberVO.getMemberEmail()));
        }
        return "/main/main";
    }
